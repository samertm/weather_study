...
def populate_db_w_city_codes(db='weather_data_OWM.db'):
    ...
    connection = sqlite3.connect(os.path.join('../', db))  # Creates a 'connection' object that points db location so that it can be connected to
    with connection:   # Provides a context manager which prevents the database from being left open after an error.  No longer need commit().  
        cursor = connection.cursor()   # cursor is an object that interacts with the database.  Handles commands to and from database. Imitates SQLite REPL
        ...
        for code in city_codes[1:-1]:
            ...
            cursor.execute(				# Executes SQLite command to populate db. 
